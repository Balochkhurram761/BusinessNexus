...
const EntrepreneurChat = () => {
  ...
  useEffect(() => {
    socket.on("receive_message", (data) => {
      if (data.senderId === selectedReceiver) {
  ...
  const getshow = async ({ id: senderId }) => {
    const user = JSON.parse(localStorage.getItem("user"));
    const token = user?.token;
    try {
      const res = await axios.get(
        `http://localhost:5000/api/auth/fetchspecfic/${senderId}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const fetchdata = async (senderId) => {
    ...
    try {
      const res = await axios.get(
        `http://localhost:5000/api/auth/fetchdata/${senderId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      ...
      setSelectedReceiver(senderId); // jis user par click hua
    } catch (error) {
